...
# This file contains a class `CachedOpenCLIPDataset` that extends the `ConfigDataset` class to add caching functionality for the labels (prompts) using a CLIP text encoder. The main features of this class include:
# - Overriding the `x_y_preprocess` method to include a caching step for the labels after preprocessing.
# - A `caching` method that converts the labels to strings, tokenizes them using the CLIP text encoder, and generates a cache for the unique tokens. This significantly speeds up training by avoiding redundant tokenization of the same labels across different samples.
# - The `get_dataloaders` method is overridden to ensure that the text encoder is available for the caching process when generating the dataloaders.
