...
# This file contains classes for creating datasets of quantum circuits. The main classes are:
# - `CircuitsConfigDatasetConfig`: a dataclass that defines the configuration for the `CircuitsConfigDataset`, including parameters like the number of qubits, gates, and the gate pool.
# - `CircuitsConfigDataset`: a dataset class that inherits from `CachedOpenCLIPDataset`, which is designed to handle quantum circuit data. It processes the input parameters and prepares the dataset for training machine learning models.
# - `MixedCircuitsConfigDatasetConfig`: a dataclass that extends `CircuitsConfigDatasetConfig` with additional parameters for handling mixed datasets.
# - `MixedCircuitsConfigDataset`: a dataset class that extends `CircuitsConfigDataset` and `MixedCachedOpenCLIPDataset`, which is designed to combine multiple datasets of quantum circuits with padding and provides collate functions for training.
