...
# This file contains helper functions for balancing a dataset of quantum circuits based on their gate length. The main functions are:
# - `get_tensor_gate_length`: computes the gate count of a tokenized circuit tensor, taking into account padding tokens.
# - `add_balance_fn_quantile_qc_length`: a function that balances a dataset according to the quantiles of the gate length distribution, with options for custom weighting and device placement. It returns a balanced set of indices for the dataset.   
